...
document.getElementById("numeri-random").innerHTML = "Numeri estratti " + numeriRandom;

//l'utente inserisce un numero per 84 tentativi
while (numeriUtente.length < (max - numBombe)) {
    
    // chiedo un numero all'utente per verificare che i numeri rispettino il range
    var numeroUtente = parseInt(prompt("Inserisci un numero da 1 a 100"));

    if (presenteInArray(numeriUtente, numeroUtente)) {
        alert("hai perso");
        document.getElementById("messaggio").innerHTML = "Ooops....hai perso!";
      // se il numero dell'utente è gia presente hai perso
    }
    else if (presenteInArray(numeriUtente, numeroUtente) == false) {
        presenteInArray.push(numeroUtente);
    } 
    else if (presenteInArray(numeriUtente, numeroUtente) == true) {
        alert("numero gia inserito");
        document.getElementById("messaggio").innerHTML = "Inserisci un altro numero!";
    }
    else if (numeriUtente.length == (max - numBombe)) {
